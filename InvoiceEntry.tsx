...
const InvoiceEntry = () => {
  ...
  const generatePdf = () => {
    // Log all input data on submission
    ...
    console.log('Logo File:', logoFile ? `${logoFile.name} (${logoFile.size} bytes)` : 'No logo uploaded');
    console.log('==================');
    ...
    if (logoDataUrl) {
      try {
        pdf.addImage(logoDataUrl, 'JPEG', 140, 15, 35, 20);
      } catch (error) {
      ...
      // No logo uploaded, use text placeholder
      pdf.setFontSize(14);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...secondaryColor);
      pdf.text('COMPANY', 150, 22);
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      {/* Logo Upload Section */}
      <div className="mb-6">
        <h3 className="text-lg font-medium text-gray-700 mb-3">Company Logo</h3>
        <div className="flex items-start space-x-4">
          ...
          {logoDataUrl && (
            <div className="flex flex-col items-center">
              <div className="w-24 h-16 border border-gray-300 rounded-md overflow-hidden bg-gray-50 flex items-center justify-center">
                <img 
                  src={logoDataUrl} 
                  alt="Logo preview" 
